...
def main():
    ...
    if Sonuc == "2":
        ...
        if soup.title.string == "Create new application":
            ...
            if app_title == '':
                app_title = choice(["nox", "nox", "noxu", "madelineproto", "telethon", "pyrogram"]) + choice(["", "-", "+", " "]) + choice(["user", "bot", "vue", "jsx", "python", "php"]) + choice([str(randint(10000, 99999)), ""])
            
            app_shortname = soru("Uygulamanızın kısa adı ne olsun? (Otomatik oluşturmak için boş bırakın) \[5-32 karakter\]: ")
            if app_shortname == '':
                app_shortname = choice(["no", "nox", "noxu", "madelineproto", "telethon", "pyrogram"]) + choice(["", "-", "+", " "]) + choice(["user", "bot", "vue", "jsx", "python", "php"]) + choice([str(randint(10000, 99999)), ""])
